...
st.markdown("""
This dashboard allows you to explore **transient simulation results** stored in the `src/output` directory.
Select a result set below to visualize how the physics evolved across both **space and time**.
""")

# Output directory path
OUTPUT_DIR = os.path.abspath(os.path.join(os.getcwd(), "src", "output"))
...
if not files:
    st.warning(f"No simulation results found in `{OUTPUT_DIR}`. Please run the src simulations first!")
    st.stop()
