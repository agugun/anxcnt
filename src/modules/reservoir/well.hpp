    ...
    void apply(Vector& residual, Matrix* jacobian, const top::IState& state_raw, double dt) override {
        ...
        if (is_injector) {
            ...
            if (jacobian) {
                ...
                (*jacobian)[r_w][r_w + 1] -= q_total * dfw_dsw;
                (*jacobian)[r_o][r_w + 1] += q_total * dfw_dsw;
            }
        }
    }
};
    ...
    void apply(Vector& residual, Matrix* jacobian, const top::IState& state_raw, double dt) override {
        ...
        for (int k = k_min; k <= k_max; ++k) {
            ...
            if (is_injector) {
                ...
                // Simplified fractional flow for multiple phases
                // For extraction, we assume production is proportional to mobilities
                double lam_w = krw / 1.0; // dummy viscs for now, should be passed or accessed
                double lam_o = krog / 2.0;
                double lam_g = krg / 0.02;
