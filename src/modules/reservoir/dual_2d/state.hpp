...
public:
    ...
    Vector variables; // Interleaved [P, Sw, P, Sw, ...]

    ReservoirDualPhase2DState(std::shared_ptr<Spatial2D> s, double p_init, double s_init)
    ...
    std::unique_ptr<IState> clone() const override {
        auto copy = std::make_unique<ReservoirDualPhase2DState>(spatial, 2000.0, 0.0);
    ...
    double sw(int c) const { return variables[2 * c + 1]; }
    double so(int c) const { return 1.0 - sw(c); }
};
