        ...
        for (int j = 0; j < ny; ++j) {
            for (int i = 0; i < nx; ++i) {
                ...
                auto add_flux = [&](int ni, int nj, double dist) {
                    ...
                    res[3 * c]     -= transmissibility * (krw / (pvt.get_mu_w(p_up) * bw_up)) * (p_n - p_c);
                    res[3 * c + 1] -= transmissibility * (krog / (pvt.get_mu_o(p_up, rs_up) * bo_up)) * (p_n - p_c);
                    double lam_g = krg / (pvt.get_mu_g(p_up) * bg_up);
                    double lam_rs = rs_up * krog / (pvt.get_mu_o(p_up, rs_up) * bo_up);
        ...
        for (int j = 0; j < ny; ++j) {
            for (int i = 0; i < nx; ++i) {
                ...
                auto add_flux_jac = [&](int ni, int nj, double dist) {
                    ...
                    double swe = (sw_u - swr) / (1.0 - swr - sorg);
                    double sge = (sg_u - sgr) / (1.0 - swc - sgr);
                    double soe = (so_u - sorg) / (1.0 - swr - sorg);

                    double dkrw_dsw = (swe > 0 && swe < 1) ? 2.0 * swe / (1.0 - swr - sorg) : 0;
                    double dkrg_dsg = (sge > 0 && sge < 1) ? 2.0 * sge / (1.0 - swc - sgr) : 0;
                    double dkro_dso = (soe > 0 && soe < 1) ? 3.0 * soe * soe / (1.0 - swr - sorg) : 0;
                    ...
                    entries.push_back({3 * c, up_col_sw, -dTw_dsw * p_diff});
                    entries.push_back({3 * c + 1, up_col_sw, -dTo_dsw * p_diff});
                    entries.push_back({3 * c + 1, up_col_sg, -dTo_dsg * p_diff});
                    entries.push_back({3 * c + 2, up_col_sw, -dTg_dsw * p_diff});
                    entries.push_back({3 * c + 2, up_col_sg, -dTg_dsg * p_diff});
                };
